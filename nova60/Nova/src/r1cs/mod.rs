...
impl<E: Engine> R1CSShape<E> {
  ...
  pub fn commit_T(
    ...
    let T = AZ
      ...
      .collect::<Vec<E::Scalar>>();
    let t_compute_elapsed = t_compute_start.elapsed();
    crate::timing::record_compute_t(t_compute_elapsed);
    println!(
      "  [timer] T_compute(len={}) took {:?}",
      T.len(),
      t_compute_elapsed
    );
    ...
    crate::timing::record_commit_t(commit_elapsed);
    println!(
      "  [timer] commit_T(len={}) took {:?}",
      T.len(),
      commit_elapsed
    );
