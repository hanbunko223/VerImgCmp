...
impl<E: Engine> NIFS<E> {
  ...
  pub fn prove(
    ...
    let commit_e_start = Instant::now();
    let comm_E = CE::<E>::commit(ck, &E, &r_E);
    crate::timing::record_neutron_commit_e(commit_e_start.elapsed());
    ...
    let (res1, res2) = rayon::join(
      || {
        ...
        let result = S.S.multiply_vec(&z1);
        crate::timing::record_neutron_multiply_vec_z1(multiply_start.elapsed());
        result
      },
      || {
        let multiply_start = Instant::now();
        let z2 = [W2.W.clone(), vec![E::Scalar::ONE], U2.X.clone()].concat();
        let result = S.S.multiply_vec(&z2);
        crate::timing::record_neutron_multiply_vec_z2(multiply_start.elapsed());
        result
      },
    );
    ...
    let prove_helper_start = Instant::now();
    ...
    crate::timing::record_neutron_prove_helper(prove_helper_start.elapsed());
    ...
    let poly_finalize_start = Instant::now();
    ...
    crate::timing::record_neutron_poly_finalize(poly_finalize_start.elapsed());

    let fold_instance_start = Instant::now();
    let U = U1.fold(U2, &comm_E, &r_b, &T_out)?;
    crate::timing::record_neutron_fold_instance(fold_instance_start.elapsed());
    let fold_witness_start = Instant::now();
    let W = W1.fold(W2, &E, &r_E, &r_b)?;
    crate::timing::record_neutron_fold_witness(fold_witness_start.elapsed());

    // return the folded instance and witness
    crate::timing::record_neutron_nifs_total(nifs_start.elapsed());
