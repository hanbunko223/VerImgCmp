...
mod tests {
  ...
  fn test_neutron_recursive_circuit_pasta() {
    test_recursive_circuit_with::<PallasEngine, VestaEngine>(&expect!["5494"]);
    test_recursive_circuit_with::<Bn256EngineKZG, GrumpkinEngine>(&expect!["5774"]);
    test_recursive_circuit_with::<Secp256k1Engine, Secq256k1Engine>(&expect!["6239"]);
  }

  #[derive(Clone, Debug)]
  struct TinyShaNoCheckCircuit<F: PrimeField> {
    preimage: Vec<u8>,
    _p: PhantomData<F>,
  }

  impl<F: PrimeField> TinyShaNoCheckCircuit<F> {
  ...
  impl<F: PrimeField> StepCircuit<F> for TinyShaNoCheckCircuit<F> {
    fn arity(&self) -> usize {
      2
    }
    ...
    fn synthesize<CS: ConstraintSystem<F>>(
      ...
      fn alloc_byte_bits_le<F: PrimeField, CS: ConstraintSystem<F>>(
        cs: &mut CS,
        value: u8,
      ...
      fn reverse_bits_within_each_byte(bits: &[Boolean]) -> Vec<Boolean> {
        assert_eq!(bits.len() % 8, 0);
        let mut reordered = Vec::with_capacity(bits.len());
        for byte in bits.chunks_exact(8) {
          reordered.extend(byte.iter().rev().cloned());
        }
        reordered
      }

      fn bits_be_to_num<F: PrimeField, CS: ConstraintSystem<F>>(
        bits: &[Boolean],
      ) -> crate::frontend::num::Num<F> {
        bits
          .iter()
          .enumerate()
          .fold(crate::frontend::num::Num::zero(), |acc, (idx, bit)| {
            let coeff = F::from(2u64).pow_vartime([(127 - idx) as u64]);
            acc.add_bool_with_coeff(CS::one(), bit, coeff)
          })
      }

      fn pack_bits_be<F: PrimeField, CS: ConstraintSystem<F>>(
        mut cs: CS,
        bits: &[Boolean],
        name: &str,
      ) -> Result<AllocatedNum<F>, SynthesisError> {
        let packed_num = bits_be_to_num::<F, CS>(bits);
        let packed = AllocatedNum::alloc(cs.namespace(|| format!("{name}_value")), || {
          packed_num
            .get_value()
      ...
      fn decompose_limb_to_bits_be<F: PrimeField, CS: ConstraintSystem<F>>(
        cs: &mut CS,
        limb: &AllocatedNum<F>,
      ) -> Result<Vec<Boolean>, SynthesisError> {
        let bit_values = limb.get_value().map(|value| {
          let repr = value.to_repr();
          let repr_bytes = repr.as_ref();
          let mut bytes = [0u8; 16];
          for i in 0..16 {
            bytes[15 - i] = repr_bytes[i];
          }
        ...
        for bit_idx in 0..128 {
          let bit = AllocatedBit::alloc(
            cs.namespace(|| format!("bit_{bit_idx}")),
            bit_values.as_ref().map(|values| values[bit_idx]),
          )?;
          bits.push(Boolean::from(bit));
        }
        let packed = bits_be_to_num::<F, CS>(&bits);
        cs.enforce(
          || "recompose_limb".to_string(),
          |_| packed.lc(F::ONE),
          |lc| lc + CS::one(),
      ...
      let mut preimage_bits = Vec::with_capacity(self.preimage.len() * 8);
      for (idx, byte) in self.preimage.iter().copied().enumerate() {
        preimage_bits.extend(alloc_byte_bits_le(
          &mut cs.namespace(|| format!("preimage_{idx}")),
          byte,
        )?);
      }
      let step_digest_bits = crate::frontend::sha256(cs.namespace(|| "step_sha"), &preimage_bits)?;
      let step_digest_input_bits = reverse_bits_within_each_byte(&step_digest_bits);

      let mut prev_state_bits = Vec::with_capacity(256);
      for (idx, limb) in z.iter().enumerate() {
        let limb_bits_be =
          decompose_limb_to_bits_be(&mut cs.namespace(|| format!("prev_limb_{idx}")), limb)?;
        prev_state_bits.extend(reverse_bits_within_each_byte(&limb_bits_be));
      }

      let mut transition_bits = prev_state_bits;
      transition_bits.extend(step_digest_input_bits);
      let next_state_bits = crate::frontend::sha256(cs.namespace(|| "next_sha"), &transition_bits)?;
      let limb0 = pack_bits_be::<F, _>(
        cs.namespace(|| "limb0"),
        &next_state_bits[..128],
        "pack_limb0",
      )?;
      let limb1 = pack_bits_be::<F, _>(
        cs.namespace(|| "limb1"),
        &next_state_bits[128..],
        "pack_limb1",
      )?;
  ...
  #[test]
  fn debug_tiny_sha_augmented_base_case_unsat_path() {
    type E1 = PallasEngine;
    type E2 = VestaEngine;

    let ro_consts = RO2ConstantsCircuit::<E1>::default();
    let circuit = TinyShaNoCheckCircuit::<<E1 as Engine>::Scalar>::new(b"alpha");
    let pp =
      crate::neutron::PublicParams::<E1, E2, TinyShaNoCheckCircuit<<E1 as Engine>::Scalar>>::setup(
        &circuit,
        &*default_ck_hint(),
        &*default_ck_hint(),
      )
    ...
    let inputs: NeutronAugmentedCircuitInputs<E1> = NeutronAugmentedCircuitInputs::new(
      ...
      vec![<E1 as Engine>::Scalar::ZERO, <E1 as Engine>::Scalar::ZERO],
      None,
      None,
      None,
    ...
    let circuit = NeutronAugmentedCircuit::new(Some(inputs), &circuit, ro_consts);
