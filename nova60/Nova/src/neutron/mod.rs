...
/// Exact per-step R1CS constraint counts for the Neutron augmented circuit.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct AugmentedConstraintTrace {
  /// Constraints created while allocating the augmented-circuit witness wrapper.
  pub create_variables: usize,
  /// Constraints for the base-case selector and base-case validity logic.
  pub base_case_logic: usize,
  /// Constraints that verify and fold the local instance into the running instance.
  pub fold_into_running_instance: usize,
  /// Constraints for incrementing the step index and selecting the local step input.
  pub step_index_and_input_select: usize,
  /// Constraints that compute and expose the augmented circuit outputs.
  pub compute_outputs: usize,
  /// Constraints belonging to the embedded application step circuit `F`.
  pub local_step_total: usize,
  /// Constraints belonging to the Neutron wrapper outside the application step circuit `F`.
  pub augmentation_total: usize,
  /// Total constraints in the full augmented recursive-step circuit.
  pub augmented_step_total: usize,
}
...
fn trace_prefix_matches(name: &str, prefix: &str) -> bool {
  name == prefix || name.starts_with(&format!("{prefix}/"))
}

/// Synthesizes the Neutron augmented circuit on a test shape CS and returns an
/// exact per-step constraint breakdown for the wrapper and embedded step circuit.
pub fn trace_augmented_constraints<E1, E2, C>(c: &C) -> Result<AugmentedConstraintTrace, NovaError>
where
  E1: Engine<Base = <E2 as Engine>::Scalar>,
  E2: Engine<Base = <E1 as Engine>::Scalar>,
  C: StepCircuit<E1::Scalar>,
{
  let ro_consts_circuit: RO2ConstantsCircuit<E1> = RO2ConstantsCircuit::<E1>::default();
  let circuit: NeutronAugmentedCircuit<'_, E1, C> =
    NeutronAugmentedCircuit::new(None, c, ro_consts_circuit);
  let mut cs: TestShapeCS<E1> = TestShapeCS::new();
  circuit.synthesize(&mut cs)?;

  let mut trace = AugmentedConstraintTrace {
    augmented_step_total: cs.num_constraints(),
    ..Default::default()
  };

  for (_, _, _, name) in &cs.constraints {
    if trace_prefix_matches(name, "F") {
      trace.local_step_total += 1;
    } else if trace_prefix_matches(name, "allocate the circuit witness") {
      trace.create_variables += 1;
    } else if trace_prefix_matches(name, "zero")
      || trace_prefix_matches(name, "Check if base case")
      || trace_prefix_matches(name, "synthesize base case")
      || trace_prefix_matches(name, "check_non_base_pass nor base_case")
      || trace_prefix_matches(name, "check_non_base_pass nor base_case = false")
    {
      trace.base_case_logic += 1;
    } else if trace_prefix_matches(name, "synthesize non base case")
      || trace_prefix_matches(name, "compute U_new")
    {
      trace.fold_into_running_instance += 1;
    } else if trace_prefix_matches(name, "i + 1")
      || trace_prefix_matches(name, "check i + 1")
      || trace_prefix_matches(name, "select input to F")
    {
      trace.step_index_and_input_select += 1;
    } else if trace_prefix_matches(name, "absorb U_new")
      || trace_prefix_matches(name, "output hash bits")
      || trace_prefix_matches(name, "convert hash to num")
      || trace_prefix_matches(name, "output new hash of this circuit")
    {
      trace.compute_outputs += 1;
    } else {
      return Err(NovaError::SynthesisError {
        reason: format!("unmatched augmented constraint path: {name}"),
      });
    }
  }

  trace.augmentation_total = trace.augmented_step_total - trace.local_step_total;

  let grouped_augmentation_total = trace.create_variables
    + trace.base_case_logic
    + trace.fold_into_running_instance
    + trace.step_index_and_input_select
    + trace.compute_outputs;
  if grouped_augmentation_total != trace.augmentation_total {
    return Err(NovaError::SynthesisError {
      reason: format!(
        "augmentation trace mismatch: grouped={grouped_augmentation_total}, total={}",
        trace.augmentation_total
      ),
    });
  }
  ...
  C: StepCircuit<E1::Scalar>,
{
  ...
  pub fn new(pp: &PublicParams<E1, E2, C>, c: &C, z0: &[E1::Scalar]) -> Result<Self, NovaError> {
    ...
    crate::timing::record_neutron_augmented_synthesize(synthesize_start.elapsed());
    let extract_start = Instant::now();
    let (l_u, l_w) = cs.r1cs_instance_and_witness(&pp.structure.S, &pp.ck)?;
    crate::timing::record_neutron_extract_instance_witness(extract_start.elapsed());
    ...
    crate::timing::record_neutron_new_base_total(new_start.elapsed());
    Ok(recursive_snark)
  }
  ...
  pub fn prove_step(&mut self, pp: &PublicParams<E1, E2, C>, c: &C) -> Result<(), NovaError> {
    let prove_step_start = Instant::now();
    ...
    crate::timing::record_neutron_prove_step_total(prove_step_start.elapsed());
    Ok(())
  }
  ...
  /// Debug helper: checks satisfiability of the current local and folded instances
  /// without enforcing the outer verification preconditions.
  pub fn debug_check_internal_sat(&self, pp: &PublicParams<E1, E2, C>) -> Result<(), NovaError> {
    let (res_r, res_l) = rayon::join(
      || pp.structure.is_sat(&pp.ck, &self.r_U, &self.r_W),
      || pp.structure.S.is_sat(&pp.ck, &self.l_u, &self.l_w),
    );
...
mod tests {
  ...
  #[test]
  fn augmented_constraint_trace_sums_to_total() {
    let tc = TrivialCircuit::<<PallasEngine as Engine>::Scalar>::default();
    let trace = super::trace_augmented_constraints::<PallasEngine, VestaEngine, _>(&tc).unwrap();
    assert_eq!(
      trace.create_variables
        + trace.base_case_logic
        + trace.fold_into_running_instance
        + trace.step_index_and_input_select
        + trace.compute_outputs,
    ...
    assert_eq!(
      trace.local_step_total + trace.augmentation_total,
      trace.augmented_step_total
    );
  }
}
