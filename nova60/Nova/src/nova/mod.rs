  ...
  C: StepCircuit<E1::Scalar>,
{
  ...
  pub fn new(pp: &PublicParams<E1, E2, C>, c: &C, z0: &[E1::Scalar]) -> Result<Self, NovaError> {
    ...
    let primary_synth_start = Instant::now();
    let zi_primary = circuit_primary.synthesize(&mut cs_primary)?;
    println!(
      "  [timer] RecursiveSNARK::new primary_synthesize took {:?}",
      primary_synth_start.elapsed()
    );
    let primary_extract_start = Instant::now();
    let (u_primary, w_primary) =
      cs_primary.r1cs_instance_and_witness(&pp.r1cs_shape_primary, &pp.ck_primary)?;
    println!(
      "  [timer] RecursiveSNARK::new primary_extract took {:?}",
      primary_extract_start.elapsed()
    );
    ...
    let secondary_synth_start = Instant::now();
    let _ = circuit_secondary.synthesize(&mut cs_secondary)?;
    println!(
      "  [timer] RecursiveSNARK::new secondary_synthesize took {:?}",
      secondary_synth_start.elapsed()
    );
    let secondary_extract_start = Instant::now();
    let (u_secondary, w_secondary) =
      cs_secondary.r1cs_instance_and_witness(&pp.r1cs_shape_secondary, &pp.ck_secondary)?;
    println!(
      "  [timer] RecursiveSNARK::new secondary_extract took {:?}",
      secondary_extract_start.elapsed()
    );
    ...
    println!(
      "  [timer] RecursiveSNARK::new total took {:?}",
      total_start.elapsed()
    );
  ...
  pub fn prove_step(&mut self, pp: &PublicParams<E1, E2, C>, c: &C) -> Result<(), NovaError> {
    ...
    if self.i == 0 {
      self.i = 1;
      println!(
        "STEP 0 TOTAL TIME: {:?} (base-step bookkeeping)",
        step_start.elapsed()
      );
      return Ok(());
    }
    let step_index = self.i;

    // fold the secondary circuit's instance
    let secondary_fold_start = Instant::now();
    ...
    println!(
      "Step {}: Secondary fold took {:?}",
      step_index,
      secondary_fold_start.elapsed()
    );
    ...
    let zi_primary = circuit_primary.synthesize(&mut cs_primary)?;
    println!(
      "  [timer] Step {} primary_synthesize took {:?}",
      step_index,
      primary_synth_start.elapsed()
    );
    ...
    let (l_u_primary, l_w_primary) =
      cs_primary.r1cs_instance_and_witness(&pp.r1cs_shape_primary, &pp.ck_primary)?;
    println!(
      "  [timer] Step {} primary_extract took {:?}",
      step_index,
      primary_extract_start.elapsed()
    );

    // fold the primary circuit's instance
    let primary_fold_start = Instant::now();
    ...
    println!(
      "Step {}: Primary fold took {:?}",
      step_index,
      primary_fold_start.elapsed()
    );
    ...
    let _ = circuit_secondary.synthesize(&mut cs_secondary)?;
    println!(
      "  [timer] Step {} secondary_synthesize took {:?}",
      step_index,
      secondary_synth_start.elapsed()
    );
    ...
    println!(
      "  [timer] Step {} secondary_extract took {:?}",
      step_index,
      secondary_extract_start.elapsed()
    );
    ...
    println!("STEP {} TOTAL TIME: {:?}", step_index, step_start.elapsed());
    Ok(())
  }
  ...
  S2: RelaxedR1CSSNARKTrait<E2>,
{
  ...
  pub fn prove(
    ...
    let final_secondary_fold_start = Instant::now();
    ...
    println!(
      "  [timer] CompressedSNARK::prove final_secondary_fold took {:?}",
      final_secondary_fold_start.elapsed()
    );
    ...
    let secondary_random_fold_start = Instant::now();
    ...
    println!(
      "  [timer] CompressedSNARK::prove secondary_random_fold took {:?}",
      secondary_random_fold_start.elapsed()
    );
    ...
    let primary_random_fold_start = Instant::now();
    ...
    println!(
      "  [timer] CompressedSNARK::prove primary_random_fold took {:?}",
      primary_random_fold_start.elapsed()
    );

    // derandomize/unblind commitments
    let derandomize_start = Instant::now();
    ...
    println!(
      "  [timer] CompressedSNARK::prove derandomize took {:?}",
      derandomize_start.elapsed()
    );

    // create SNARKs proving the knowledge of Wn primary/secondary
    let snark_primary_start = Instant::now();
    let snark_secondary_start = Instant::now();
    let (snark_primary, snark_secondary) = rayon::join(
      || {
        ...
        println!(
          "  [timer] CompressedSNARK::prove S1::prove took {:?}",
          snark_primary_start.elapsed()
        );
        res
      },
      || {
        ...
        println!(
          "  [timer] CompressedSNARK::prove S2::prove took {:?}",
          snark_secondary_start.elapsed()
        );
        res
      },
    );
    ...
    println!(
      "  [timer] CompressedSNARK::prove total took {:?}",
      total_start.elapsed()
    );
