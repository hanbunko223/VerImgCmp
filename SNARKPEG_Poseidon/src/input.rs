...
pub const DCTQ_STEP_ROWS: usize = 16;
...
impl DctqInput {
    ...
    pub fn into_steps(self) -> Vec<DctqStep> {
        self.rows
            ...
            .map(|rows| {
                rows.try_into()
                    .expect("validated input always splits into fixed-size steps")
            })
            .collect()
    }
}
...
mod tests {
    ...
    fn resolution_specs_match_expected_counts() {
        let expected = [
            ("SD", 640, 480, 240, 240, 1920, 120, 0),
            ("HD", 1280, 720, 720, 720, 5760, 360, 0),
            ("FHD", 1920, 1080, 1620, 1620, 12960, 810, 0),
            ("QHD", 2560, 1440, 2880, 2880, 23040, 1440, 0),
            ("4K", 3840, 2160, 6480, 6480, 51840, 3240, 0),
        ];
    ...
    #[test]
    fn fhd_padding_is_zero_with_two_logical_rows_per_step() {
        assert_eq!(FHD_SPEC.padded_pixels, 0);
        assert_eq!(FHD_SPEC.step_count, 810);
    }
