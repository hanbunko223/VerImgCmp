...
pub enum ProverError {
    ...
    #[error("this native SNARKPEG_Poseidon branch currently supports only `dctq`")]
    UnsupportedFunction,
...
pub fn prove_spartan_compressed(
    ...
    let _setup_s = setup_start.elapsed().as_secs_f64();

    let compression_start = Instant::now();
    let compressed_snark = SpartanCompressedSNARK::prove(pp, &pk, recursive_snark)?;
    let _compression_s = compression_start.elapsed().as_secs_f64();
