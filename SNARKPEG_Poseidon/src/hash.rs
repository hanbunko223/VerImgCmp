...
pub fn pack_step_pixels(step: &DctqStep) -> PackedPixelsStep {
    (0..DCTQ_STEP_ROWS)
        ...
        .into_boxed_slice()
        .try_into()
        .expect("step always packs into a fixed row count")
}
...
pub fn pack_step_chunks(packed_pixels: &[PackedPixelsRow; DCTQ_STEP_ROWS]) -> PackedChunksStep {
    (0..DCTQ_STEP_ROWS)
        ...
        .into_boxed_slice()
        .try_into()
        .expect("step always packs into a fixed chunk-row count")
}
...
pub fn reduce_row_hashes(row_hashes: &[Scalar; DCTQ_STEP_ROWS]) -> Scalar {
...
mod tests {
    ...
    #[test]
    fn reduce_row_hashes_uses_two_poseidon8_groups_then_poseidon2_root() {
