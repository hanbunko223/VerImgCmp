...
fn print_neutron_timing_summary(
    ...
    let prove_step_leaf_sum = timing.neutron_commit_e
        ...
        + timing.commit_w
        + extract_other_s;
    let prove_step_other_s = (prove_step_s - prove_step_leaf_sum).max(0.0);
    ...
    println!("| others                        | {:>11.6} |", prove_step_other_s);
    println!("+-------------------------------+-------------+");
...
fn run() -> Result<(), ProverError> {
    ...
    let artifact = RecursiveProofArtifact::<NativeRecursiveSNARK> {
        backend: "neutron-lh-90".to_string(),
