...
mod tests {
    ...
    #[test]
    fn into_steps_splits_hd_rows_into_90_steps() {
        let rows = (0..DCTQ_HD_ROWS)
            .map(|row_idx| {
                std::array::from_fn(|col_idx| {
                    [
                        (row_idx % 256) as u8,
                        (col_idx % 256) as u8,
                        ((row_idx + col_idx) % 256) as u8,
                    ]
                })
            })
            .collect::<Vec<DctqRow>>();

        let input = DctqInput { rows };
        let steps = input.into_steps();

        assert_eq!(steps.len(), DCTQ_HD_STEP_COUNT);
        assert_eq!(steps[0].len(), DCTQ_STEP_ROWS);
        assert_eq!(steps[0][0][0], [0, 0, 0]);
        assert_eq!(
            steps[DCTQ_HD_STEP_COUNT - 1][DCTQ_STEP_ROWS - 1][DCTQ_HD_WIDTH - 1],
            [
                ((DCTQ_HD_ROWS - 1) % 256) as u8,
                ((DCTQ_HD_WIDTH - 1) % 256) as u8,
                ((DCTQ_HD_ROWS + DCTQ_HD_WIDTH - 2) % 256) as u8,
            ]
        );
    }
}
