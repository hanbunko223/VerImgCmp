...
pub fn prove(
    ...
    println!(
        "Running NeutronNova with native Rust lattice-hash frontend plus internal DCTQ checks and engine: {}",
        std::any::type_name::<PrimaryEngine>()
    );
    println!(
        "Warning: using experimental NeutronNova backend from local nova60/Nova; this branch enforces byte range checks, internal DCTQ linear layers, and lattice hashing without a compressed proof."
    );
...
mod tests {
    ...
    use crate::{
        ...
        hash::{flatten_step_pixels, lattice_hash_channels, pack_pixel, step_digest},
    ...
    fn step_digest_changes_when_pixel_changes() {
        ...
        let before_lattice = lattice_hash_channels(&flatten_step_pixels(&synthetic_step(3)));
        let after_lattice = lattice_hash_channels(&flatten_step_pixels(&step));
        assert_ne!(before_lattice, after_lattice);
